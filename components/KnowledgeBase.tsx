...
const KnowledgeBase: React.FC<Props> = ({ documents, onAddDocument, onRemoveDocument, initialSource, onClearInitial }) => {
  ...
  const handleChat = async () => {
    if (!userQuery.trim() || documents.length === 0) return;
    ...
    try {
      const response = await chatWithNotebook(userQuery, documents, chatHistory);
           ...
           {activeTab === 'sources' && (
             <div className="flex-1 flex flex-col p-4 md:p-6 min-h-0">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                   <h3 className="text-xl md:text-2xl font-black text-white">Sources ({documents.length})</h3>
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar">
                   {documents.length === 0 ? (
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                       {documents.map(doc => (
                         <div key={doc.id} className="bg-gray-950 border border-gray-800 rounded-xl p-4 flex flex-col hover:border-emerald-500/30 transition-all">
                            <div className="flex justify-between items-start mb-3">
                               <h4 className="font-bold text-white text-sm truncate pr-4">{doc.title}</h4>
                               <button onClick={() => onRemoveDocument(doc.id)} className="text-gray-600 hover:text-red-500 transition-colors shrink-0">
                                  <Trash2 size={14} />
                               </button>
                            </div>
